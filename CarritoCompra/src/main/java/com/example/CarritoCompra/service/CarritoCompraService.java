package com.example.CarritoCompra.service;
...
import com.example.CarritoCompra.client.*;
import com.example.CarritoCompra.model.*;
import com.example.CarritoCompra.repository.*;
...
public class CarritoCompraService {
    ...
    @Autowired
    private CarritoCompraRepository repo;

    @Autowired
    private PeliculaClient peliculaClient;

    public ResponseEntity<?> registrarCompra(@RequestBody CarritoCompra c){
    ...
    public ResponseEntity<?> eliminarCompra(@PathVariable Long id){
        CarritoCompra existe = repo.findById(id).orElse(null);
        if(existe == null){
    ...
    public ResponseEntity<?> actualizarCompra(@RequestBody CarritoCompra c, @PathVariable Long id){
        CarritoCompra existe = repo.findById(id).orElse(null);
        if(existe == null){
            return ResponseEntity.badRequest().body("Compra no Encontrada!");
        }
